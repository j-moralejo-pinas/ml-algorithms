...
def plot_unsupervised_learning_data_and_model(
    ...
    unique_clusters = list({float(c) for c in cluster_assignments})
    ...
    cluster_color_map = {cluster_id: colors[int(cluster_id)] for cluster_id in unique_clusters}
    ...
    for idx, centroid in enumerate(centroids):
        darker_color = _darken_color(colors[idx])
        plt.scatter(centroid[0], centroid[1], color=darker_color, marker="X", s=200)
